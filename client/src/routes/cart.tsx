...
function CartPage() {
    ...
    const deviceQueries = cartItems.map((item) => queryClient.getQueryData(deviceQueryOptions(item.deviceId).queryKey));
    ...
    const removeMutation = useMutation({
        mutationFn: (deviceId: number) => (user ? removeFromCartFn(user?.id, deviceId) : Promise.resolve()),
        onSuccess: () => {
                    ...
                    {cartItems.map((item, index) => (
                        <div key={item.id}>
                            <CartItem
                                ...
                                isRemoving={removeMutation.isPending && removeMutation.variables === item.deviceId}
                            />
                ...
                <div className="flex flex-col gap-4 rounded-lg border p-4 lg:self-start">
                    ...
                    <AlertDialog>
                        <AlertDialogTrigger asChild>
                            <Button variant="outline" className="w-full" disabled={clearMutation.isPending}>
                                Clear Cart
                            </Button>
                        </AlertDialogTrigger>
                        <AlertDialogContent>
                            ...
                            <AlertDialogFooter>
                                ...
                                <AlertDialogAction onClick={() => clearMutation.mutate()}>Clear</AlertDialogAction>
                            </AlertDialogFooter>
                        </AlertDialogContent>
                    </AlertDialog>
                </div>
